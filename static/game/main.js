...
async function showBoard(turnCount, previousDisc) {
  ...
  showWarningDiscMessage(previousDisc, nextDisc, winnerDisc)
...
function showWarningDiscMessage(previousDisc, nextDisc, winnerDisc) {
...
function warningMessage(previousDisc, nextDisc, winnerDisc) {
  if (nextDisc !== null) {
    ...
    return winnerDisc === WINNER_DRAW
      ? '引き分けです'
      : `${discToString(winnerDisc)}の勝ちです`
  }
}
